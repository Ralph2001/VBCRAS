<template>
  <div class="flex items-center" v-for="(value, key) in options" :key="key">
    <input :id="props.name" type="radio" :value="key" :name="name" :class="{
    ...
    <label :for="props.name" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ value }} <span
        v-if="error && nolabel" class="text-red-600">*</span></label>
  </div>
</template>
