<template>
    <div class="grid grid-cols-4 sm:px-5 md:lg:px-20 relative">
        <input
            :value="type"
            disabled
            :class="[props.type === 'CCE' ? 'bg-[#4A90E2]' : 'bg-[#2C7A7B]']"
        ...
        <input
            ...
            :value="props.petition_number_value"
        ...
        <input
            disabled
            :tabindex="skip ? '-1' : '0'"
            @input="emit('type-year', $event.target.value)"
            :class="[
                props.republic_act === '10172' ? 'border-e-transparent' : ''
            ]"
            v-model="year"
