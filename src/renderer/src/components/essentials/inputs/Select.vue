<template>
    <div class="w-[10rem]">
        ...
        <select id="countries" @change="emit('update:modelValue', $event.target.value)" :tabindex="skip ? '-1' : ''"
            ...
            <option v-for="option in options" :key="option" :value="option" :selected="modelValue === option">{{ option
                }}
            </option>
        </select>
    </div>
