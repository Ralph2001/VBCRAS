<template>
    <div class="fixed inset-0 bg-white z-50 flex flex-col h-screen">
        <!-- Header -->
        <div
            class="p-4 border-b flex items-center justify-between bg-gray-100 shadow-sm"
        >
            <div class="flex flex-col">
                <h2 class="font-semibold text-lg text-gray-800 truncate">
                    {{ props.fileInfo.name?.replace('.pdf', '') || 'Unnamed' }}
                </h2>
                <div class="space-x-4 truncate text-xs"></div>
            </div>
            <button
                @click="emits('close-details')"
        ...
        <!-- Info Bar -->
        <div
            class="flex items-center justify-between px-4 py-3 bg-gray-100 border-b shadow text-sm text-gray-600"
        >
            <div class="flex items-center space-x-2">
                <button
                    @click="isFilterOpen = !isFilterOpen"
                    :disabled="isCertifiedTrueCopy"
                    :class="
                        isFilterOpen
            ...
            <div class="flex justify-center items-center gap-4 my-2 text-sm">
                ...
                <span class="text-gray-700"
                    >Page {{ currentPageNum }} / {{ totalPages }}</span
                >
            ...
            <div class="flex items-center space-x-2">
                <label
                    ...
                    :class="
                        isFilterOpen ? 'bg-gray-300 cursor-not-allowed' : ''
                    "
                ...
                >
                    <input
                        ...
                        "
                        type="checkbox"
                        v-model="isCertifiedTrueCopy"
                        class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <span class="text-sm text-gray-800 font-medium"
                        >Certified True Copy</span
                    >
                </label>
        ...
        <div class="flex-1 flex flex-row overflow-hidden">
            ...
            >
                <div
                    class="min-w-full min-h-full p-4"
                ...
                >
                    ...
                    <div
                        v-if="isCertifiedTrueCopy"
                        v-for="(box, index) in boxes.filter(
                            (b) => b.page === currentPageNum
                        )"
                        :key="index"
                        class="absolute flex flex-col cursor-move text-center overflow-hidden"
                        :style="{
            ...
            <div class="w-72 p-4 border-l bg-white h-full" v-if="null">
                ...
                <div class="mb-4">
                    <label
                        for="position"
                        class="block text-xs text-gray-800 font-semibold mb-1"
                        >Predefined Position</label
                    >
                    <select
                        id="position"
                        v-model="position"
                    ...
                    >
                        ...
                        <option
                            :value="pos.value"
                            v-for="pos in predefinedPositions"
                            :key="pos.name"
                        >
                            {{ pos.name }}
                        </option>
                ...
                <!-- Color Picker -->
                <label class="block text-xs text-gray-800 font-semibold mb-2"
                    >Change Stamp Color</label
                >
            ...
            <div
                v-if="isFilterOpen"
                class="w-72 p-3 border-l bg-white h-full text-sm text-gray-700"
            >
                <h3 class="font-medium mb-2">Filters</h3>

                <!-- Presets -->
                <div class="mb-4">
                    <label for="preset" class="block mb-1 text-xs font-medium"
                        >Preset</label
                    >
                    <select
                        id="preset"
                ...
                <template v-for="control in filterControls" :key="control.key">
                    <div class="mb-3">
                        <label :for="control.key" class="block mb-1 text-xs">
                            {{ control.label }} ({{ filters[control.key]
                            }}{{ control.unit }})
                        </label>
                        <input
                            type="range"
                            ...
                            :max="control.max"
                            :step="control.step"
                            v-model.number="filters[control.key]"
                            class="w-full"
                        />
                    </div>
                </template>
...
const boxes = ref([{ x: 360, y: 832, width: 200, height: 120, page: 1 }])

watchEffect(() => {
    if (position.value) {
        const selectedPosition = predefinedPositions.find(
...
const handlePreviewPrint = async () => {
    try {
        ...
        const path = await window.ScannedApi.OpenInSideBar(
            props.fileInfo.filepath
        )
        const cleanBase64 = path.fileUrl.replace(
        ...
        if (isCertifiedTrueCopy.value) {
            for (const box of boxes.value) {
                // Use for...of for easier async/await if needed later, though not strictly required here.
                ...
                textChar.forEach((line, index) => {
                    ...
                    if (index < textChar.length - 1) {
                        // Add spacing only between lines
                ...
                let currentYPositionForText =
                    pdfY_bottom + height - (height - totalContentHeight) / 2

                textChar.forEach((line, index) => {
                    const font =
                        line.fontWeight === 'bold' ? helveticaBold : helvetica
                    ...
                    if (index > 0) {
                        currentYPositionForText -=
                            textChar[index - 1].fontSize + lineSpacing
                    }
...
const renderPdf = async () => {
    if (
        ...
        currentPageNum.value > totalPages.value
    )
        return
...
const loadPdf = async () => {
    ...
    const cleanBase64 = pdfbase64.value.replace(
        /^data:application\/pdf;base64,/,
        ''
    )
...
const canvasFilter = computed(() => {
    const { brightness, contrast, saturate, grayscale, sepia, invert } =
        filters.value
    return [
