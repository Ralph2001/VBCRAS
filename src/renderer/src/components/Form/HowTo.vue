<template>
    <div class="fixed inset-0 bg-white z-50 flex items-start justify-center overflow-y-auto">
        <div class="bg-white w-full max-w-3xl rounded-xl p-8">
            <!-- Header -->
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-bold text-gray-800">📘 Step-by-Step Guide: Filing a Clerical Error Petition
                </h1>
                <button @click="close" class="text-red-600 hover:text-red-800 font-bold text-2xl">&times;</button>
            </div>

            <!-- Instructions -->
            <div class="space-y-10 text-gray-800 text-base leading-relaxed">

                <div v-for="(step, index) in steps" :key="index" class="space-y-4">
                    <h2 class="text-xl font-semibold">Step {{ index + 1 }}: {{ step.title }}</h2>
                    <img :src="step.image" alt="Step Image" class="w-full rounded border" />
