<template>
  <div class="flex flex-col h-full border rounded-lg relative justify-center">
    ...
    <div class="flex mt-2 basis-[5%] flex-row justify-cente6r h-16 items-center gap-2 p-2 w-full">
      ...
      <div v-if="searchData.length && searchQuery != ''" class="flex p-1 items-center gap-2 justify-center">
        <p class="text-xs font-mono text-gray-800">
    ...
    <div class="flex basis-[94%] border flex-col p-4 overflow-y-scroll relative h-[calc(100vh-350px)]">
      ...
      <div class="h-full flex flex-row" v-if="type && year && month && searchQuery == ''">
        <div :class="{ 'sm:hidden md:lg:block h-full': pdfSource }" class="h-full grow">
          ...
          <RecycleScroller v-if="type && year && month && !searchQuery" :items="files" class="h-full border"
            :item-size="18" key-field="name" v-slot="{ item }">
            <li @dblclick.prevent="openFile(item.filepath, item.name, item.id)" tabindex="0" :class="{
              'bg-blue-500 hover:bg-blue-500 text-white group ': ActiveFile === item.name && pdfSource,
              ...
              <div class="block w-[60%] overflow-hidden truncate">
                ...
                {{ item.name ? item.name.replace(".pdf", "") : 'Unnamed' }}
              </div>
...
const filteredData = computed(() => {
  let filtered = props.data.filter((item) => {
    ...
    if (!year.value) return item.scanned_type.name === type.value;
    if (!month.value) return item.scanned_type.name === type.value && item.year === year.value;
    return (
      item.scanned_type.name === type.value && item.year === year.value && item.month === month.value
    );
  });
...
const types = computed(() => {
  return [...new Set(filteredData.value.map((data) => data.scanned_type.name))].sort((a, b) => a - b);
});
