    ...
    <div class="h-[calc(100vh-180px)] flex flex-col border rounded">
        <ScannedHeader>
            <template #content>
                ...
                <div class="ml-auto w-32 flex items-center justify-center">
                    <button
                        @click="emits('openNewTypeModal')"
                        v-if="selected_type === null"
                        class="rounded-md w-fit px-2.5 h-fit bg-blue-600 text-white py-1.5 text-xs font-medium"
                    >
                        Add Type
                    </button>
                </div>
        ...
        <div class="h-full overflow-y-auto relative p-4 flex flex-col">
            ...
            <div
                ...
                v-if="documents.loading && !documents.isError"
            >
                <p class="font-medium font-mono">
                    Retrieving information, please hold on...
                </p>
            </div>
            <!-- Error State -->
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <font-awesome-icon
                            icon="fa-solid fa-folder"
                            class="text-yellow-400 text-5xl z-10"
                        />
                    </div>
                    <p
                        class="text-xs text-center font-medium text-gray-800 leading-tight truncate w-full max-w-[6rem]"
                    >
                        {{ type.name }}
                    </p>
                </button>
            ...
            <RecycleScroller
                class="flex-1"
                v-if="
                    isValidSelection && !documents.loading && !documents.isError
                "
                :items="filteredFiles"
                :item-size="5"
                key-field="id"
...
const getTypeName = computed(() => {
    const type = scannedType.value.find(
        (type) => type.id === selected_type.value
    )
...
const directory = computed(() => {
    ...
    if (selected_year.value === null) {
        return [...new Set(documents.scanned.map((data) => data.year))].sort(
            (a, b) => a - b
        )
    }

    if (selected_month.value === null) {
        return [...new Set(documents.scanned.map((data) => data.month))].sort(
            (a, b) => a - b
        )
    }
