<template>
    <div class="flex flex-col h-full border rounded-lg relative">
        ...
        <div
            class="grid grid-cols-3 items-center justify-between px-3 py-3 bg-gray-50/50"
        >
            ...
            <div class="relative">
                ...
                <input
                    ...
                    v-model="searchQuery"
                    :class="{
                        'border-blue-600 border ring-2 ring-blue-400':
                            searchQuery != '',
                    }"
        ...
        <div
            class="flex flex-row justify-center h-16 items-center gap-2 mt-5 px-3 py-3 w-full"
        >
            <button
        ...
        <div class="flex flex-col p-4 overflow-y-scroll relative h-full">
            <ul
                class="w-full space-y-1 px-2 m list-none list-inside dark:text-gray-400"
            >
                <li
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            <p
                                class="text-sm italic text-gray-600 font-normal mr-2"
                            >
                                {{ item.type }} {{ item.month }}
                                {{ item.year }}
                                <font-awesome-icon
                                    @click="openPath(item.filepath, item.name)"
                ...
                <div class="h-[calc(100vh-316px)]" v-if="searchQuery != ''">
                    ...
                    >
                        <p
                            class="text-center text-lg font-bold text-gray-600 font-italic"
                        >
                            No files found.
                        </p>
...
const filteredData = computed(() => {
    let filtered = props.data.filter((item) => {
        ...
        if (!month.value)
            return item.type === type.value && item.year === year.value
        return (
...
const types = computed(() => {
    return [...new Set(filteredData.value.map((data) => data.type))].sort(
        (a, b) => a - b
    )
})

const years = computed(() => {
    return [...new Set(filteredData.value.map((data) => data.year))].sort(
        (a, b) => a - b
    )
})
