<template>
  <div class="flex flex-col h-full border rounded-lg relative justify-center">
    ...
    >
      <p class="text-xl tracking-wider font-semibold ms-3 text-gray-800">
        Scanned Documents
      </p>
      ...
      <!-- <div class="flex flex-row gap-1 justify-end pr-5">
        ...
        <div
          :class="{
            'text-gray-600 shadow-custom-inset': Documents.OpenMode === 'OpenNewWindow',
          }"
          class="bg-white px-1 text-gray-400 rounded hover:cursor-pointer active:scale-95"
    ...
    <div
      class="flex mt-2 basis-[5%] flex-row justify-cente6r h-16 items-center gap-2 p-2 w-full"
    >
      <button
      ...
      <div
        class="flex flex-row p-1 items-center gap-2 justify-center"
      ...
      <div
        v-if="searchData.length && searchQuery != ''"
        class="flex p-1 items-center gap-2 justify-center"
      >
        <p class="text-xs font-mono text-gray-800">
          {{ searchData.length.toLocaleString("en-US") }}
        </p>
        <p class="text-xs font-mono text-gray-800 block">items</p>
      </div>
    </div>

    <div
      class="flex basis-[94%] border flex-col p-4 overflow-y-scroll relative h-[calc(100vh-350px)]"
    >
      <ul class="w-full space-y-1 px-2 m list-none list-inside dark:text-gray-400">
        <!-- Folders Here -->
        <li
          v-if="!type && searchQuery == ''"
          v-for="type in types"
          :key="type"
        ...
        >
          <font-awesome-icon
            ...
            class="text-yellow-400/70 me-2 ms-3"
          />
          {{ type }}
        </li>

        <li
          v-if="!data.length && searchQuery == '' && !props.loading"
          class="text-center text-gray-600 font-italic"
        >
          Empty Data.
        </li>

        <li
          v-if="type && !year && searchQuery == ''"
          v-for="year in years"
          :key="year"
      ...
      <div class="h-full flex flex-row" v-if="type && year && month && searchQuery == ''">
        <div :class="{ 'sm:hidden md:lg:block h-full': pdfSource }" class="h-full grow">
          ...
          >
            <li
              @dblclick.prevent="openFile(item.filepath, item.name, item.id)"
              tabindex="0"
              :class="{
                'bg-blue-500 hover:bg-blue-500 text-white group ':
                  ActiveFile === item.name && pdfSource,
                'bg-blue-100/90 ': matchToOpened(item.id) && ActiveFile !== item.name,
              }"
            ...
            >
              <div class="block w-[60%] overflow-hidden truncate">
                <font-awesome-icon
                  ...
                  class="text-red-500 me-2 ms-3"
                />
                {{ item.name.replace(".pdf", "") }}
              </div>
              <p
                :class="{ 'text-white  ': ActiveFile === item.name && pdfSource }"
        ...
        <div
          :class="{ 'flex flex-col': pdfSource, 'sm:hidden md:lg:flex': !pdfSource }"
        ...
        >
          <div class="flex flex-row items-center w-full h-[2rem] gap-2" v-if="pdfSource">
            <button
              v-if="pdfSource && !full_screen"
              @click="pdfSource = ''"
              class="border px-2 py-1.5 h-full self-start w-[5rem] text-white text-xs font-medium bg-red-400 hover:bg-red-500 rounded hover:text-white"
            >
              Close
            </button>
            <button
              :title="!full_screen ? 'Expand' : 'Shrink'"
              v-if="pdfSource"
          ...
          <div
            ...
            :class="{
              hidden: !pdfSource,
              ' fixed gap-2 top-0 bottom-0 right-0 left-0 z-50 flex flex-col px-5 py-2 bg-gray-700': full_screen,
              flex: !full_screen,
            }"
            class="w-full h-full"
          >
            <button
              ...
              v-if="full_screen"
              @click="full_screen = !full_screen"
              class="justify-center px-2 py-1 self-end w-[3rem] shadow-lg rounded-sm text-gray-800 text-sm font-medium bg-white hover:bg-gray-800 hover:text-white"
            >
              <font-awesome-icon icon="fa-solid fa-expand" v-if="!full_screen" />

              <font-awesome-icon icon="fa-solid fa-compress" v-if="full_screen" />
            </button>

            <div
              :class="{ 'top-[3rem]': full_screen, 'top-[4rem]': !full_screen }"
              v-if="pdfSource"
            ...
            <div class="w-full h-full bg-[#323639]">
              <iframe :src="pdfSource" frameborder="0" class="w-full h-full"> </iframe>
            </div>
          </div>

          <div
            v-else
            class="h-full w-full flex items-center justify-center shadow-sm bg-gray-100 relative"
          >
            <p
              class="text-gray-600 font-mono text-lg tracking-wider bottom-[8rem] font-semibold"
            >
              VBCRAS
            </p>
          </div>
      ...
      <div class="h-full flex flex-row" v-if="searchQuery != ''">
        <div :class="{ 'sm:hidden md:lg:block h-full': pdfSource }" class="h-full grow">
          ...
          >
            <li
              tabindex="0"
              @dblclick="openFile(item.filepath, item.name)"
            ...
            >
              ...
              <p
                class="text-sm italic text-gray-600 font-normal group-hover:mr-5 duration-300 mr-2"
              >
...
onClickOutside(settings, (event) => (ScannedSettings.value = false));
