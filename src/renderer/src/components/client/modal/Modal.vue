<template>
  ...
  >
    ...
    >
      <div
        class="relative bg-white h-full rounded-sm w-full overflow-y-auto dark:bg-gray-700"
      >
        <!-- Header -->
        <div class="flex bg-white items-center top-0 right-0 left-0 p-3 border-b">
          <h3
            v-if="!titleCard"
            class="text-sm tracking-wide font-bold text-gray-800 uppercase dark:text-white"
          >
            {{ label }}
          </h3>
          ...
          <div class="p-2 truncate overflow-ellipsis" v-if="titleCard">
            <h3
              class="text-sm font-semibold text-gray-900 dark:text-white tracking-wide truncate"
            >
              {{ first }}
            </h3>
            <h3
              class="text-xs font-semibold pl-2 text-blue-500 dark:text-white tracking-wider"
            >
        ...
        <!-- Footer -->
        <div
          :class="[titleCard ? ' h-0' : '', $attrs.footerBG]"
