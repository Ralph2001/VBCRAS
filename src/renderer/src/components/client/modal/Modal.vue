<template>
    <div tabindex="-1"
        ...
        <div :class="{ ' max-w-full h-[calc(100vh-20px)] overflow-y-hidden': large, 'max-w-2xl max-h-full': medium }"
            class="relative p-4 w-full  ">

            <div class="relative bg-white h-full  overflow-y-auto  rounded-lg shadow dark:bg-gray-700 ">
                <div class="flex top-0 right-0 left-0 border-b p-3">
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white"> {{ label }} </h3>
                    <slot name="header"></slot>
                </div>
                <div class="p-3 md:p-4 h-[calc(100vh-180px)] overflow-y-scroll ">
                    <slot>
                    </slot>
                </div>
                <div class="flex items-center justify-end p-2 md:p-3 rounded-b border-t dark:border-gray-600">
