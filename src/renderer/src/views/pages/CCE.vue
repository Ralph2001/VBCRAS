<template>
    <div class="flex flex-col relative justify-center w-full p-10 CCEMAIN">
        ...
        >
            <div
                class="w-full gap-2 flex flex-col md:flex-row items-center justify-center"
            >
                <Button
                    label="Create New Petition"
                    isActive
                    :class="`rounded`"
                    @click="open_modal()"
                />
          ...
          <font-awesome-icon icon="fa-solid fa-gear" /></button> -->
                <HowTo />
                <IsPathAccessible
                    :filePath="system_setting.defaults.file_path"
        ...
        >
            <div
                class="h-[20rem] w-full max-w-screen-md p-4 rounded-md bg-white shadow flex flex-col gap-2"
            >
                ...
                <div class="flex-1 p-1 gap-2 flex flex-col">
                    ...
                    <div class="p-4 bg-gray-100 shadow-inner rounded-md">
                        <p class="text-wrap text-sm">
                            {{ system_setting.defaults[0].file_path }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <AlertPath
            v-if="alertmodal"
            :title="alertmodal_title"
            :body="alertmodal_body"
        />
        <!-- v-if="is_validating" -->
        <ValidateClericalPopup
            :isRegen="isAtRegen"
            v-if="is_validating"
            :path="last_saved_filepath"
            @cancel="cancel_validating_stage"
            @proceed="create_validated_document"
            @cancel-regen="cancel_regen_stage"
        />
        <!--  v-if="is_validating"  -->
        ...
        <div class="h-[calc(100vh-250px)] relative">
            <div class="w-full flex flex-row p-1 items-center justify-between">
                <div class="flex flex-col">
                    ...
                    >
                        <div
                            class="h-2 bg-[#4A90E2] block rounded-sm w-4"
                        ></div>
                    ...
                    >
                        <div
                            class="h-2 bg-[#2C7A7B] block rounded-sm w-4"
                        ></div>
        ...
        <DocumentStatusModal
            :visible="is_creating"
            :is-edit="is_document_edit_mode"
            :is-regenerating="is_document_regenerating"
        />
        <RegenerateMessage
            v-if="is_regen"
            @proceed="handleRegenerate"
            @cancel="is_regen = false"
            :data="regen_data"
        />

        <Modal
            large
            footerBG="bg-white  border-t  border-gray-300"
        ...
        <button
          ...
          @click="close_modal"> <font-awesome-icon icon="fa-solid fa-arrow-left" /> Return</button>
            ...
            >
                <div
                    :class="[backround_per_event]"
                    class="h-full flex flex-col py-4 gap-2"
                >
                    <!-- 1st  Document Selector-->
                    <div
                        class="grid sm:grid-cols-1 md:lg:grid-cols-2 gap-2 items-start w-full justify-center p-2"
                        ref="isFormVisible"
                    >
                        <Box title="Document Type" width="w-full">
                            <p
                                class="text-sm text-gray-400"
                                v-if="is_document_edit_mode"
                            >
                                Fields cannot be changed while in Edit Mode.
                            </p>
                            <div
                                class="grid grid-cols-2 flex-wrap p-2 gap-3 items-center justify-center w-full"
                                ref="documentChanger"
                                tabindex="-1"
                            >
                                <!-- Select components with change handlers properly defined -->
                                <Select
                                    :readonly="is_document_edit_mode"
                                    :error="v$.republic_act_number.$error"
                                    skip
                                    :options="republic_act"
                                    v-model="formData.republic_act_number"
                                    label="Republic Act"
                                    @change="PublicationSetter"
                                />
                                <Select
                                    :readonly="is_document_edit_mode"
                                    :error="v$.petition_type.$error"
                                    skip
                                ...
                                />
                                <div
                                    class="flex flex-row items-center gap-2 ml-4 mt-4"
                                >
                                    <CheckBox
                                        :readonly="is_document_edit_mode"
                                        skip
                                        v-model="formData.is_migrant"
                        ...
                        <Box title="Document Header" width="w-full">
                            <div class="flex flex-col gap-2 w-full">
                                <InputAutoComplete
                                    label="Province"
                                    @change="formData.header_municipality = ''"
                                    :error="v$.header_province.$error"
                                    v-model="formData.header_province"
                                    :suggestion_data="province"
                                />
                                <InputAutoComplete
                                    label="City/Municipality"
                                    :error="v$.header_municipality.$error"
                                    cap
                                    v-model="formData.header_municipality"
                                    :suggestion_data="municipality_of"
                                />
                            </div>
                        </Box>
                    </div>
                    <!-- 2nd Header-->
                    <div
                        class="flex flex-col gap-2 overflow-y-scroll py-3 mt-5 px-10"
                    >
                        <HeaderCCE
                            :petition_type="formData.petition_type"
                            :type_label="document_type_label"
                        />
                    </div>
                        ...
                        <Box title="Petition Details" width="w-full">
                            <div class="grid grid-cols-1 w-full gap-2">
                                <div class="w-full flex flex-col">
                                    ...
                                    >
                                    <MultiInput
                                        @type-petition-number="
                                            change_petitioner_number
                                        "
                                        @type-year="change_petitioner_date"
                                        skip
                                        :migrant="formData.is_migrant"
                                        :republic_act="is_default_republic_act"
                                        :type="is_default_petition_type"
                                        :petition_number_value="
                                            is_default_petitioner_number
                                        "
                                    />
                                </div>
                                <Input
                    ...
                    <div
                        class="flex sm:flex-col md:lg:flex-row flex-wrap gap-2 items-stretch"
                    >
                        <div
                            class="basis-[35%]"
                            v-if="
                        ...
                        >
                            ...
                            >
                                <div class="grid grid-rows-2 gap-2 w-full">
                                    <Selector
                                        @change_="
                                            change_document_owner_relation()
                                        "
                                    ...
                                    <div class="w-full flex flex-col gap-2">
                                        ...
                                        >
                                            <span class="font-medium"
                                                >Note:</span
                                            >
                                            If the petitioner's name differs
                                            from the one on the birth
                                            certificate, please enter the name
                                            exactly as it appears on the birth
                                            certificate in this field. If the
                                            names match, leave the box checked.
                                        </p>
                                        <div
                                            v-if="null"
                    ...
                    {{ formData.event_type }} -->
                                        <Input
                                            v-if="formData.petitioner_error_in"
                                            :readonly="
                                                formData.petitioner_error_in ===
                                                    'my' &&
                                                (formData.event_type ===
                    ...
                    <div class="flex sm:flex-col md:lg:flex-row gap-2">
                        ...
                        <div class="basis-[45%]">
                            <Box title=", at" width="w-full ">
                                <div
                                    class="grid sm:grid-cols-1 lg:grid-cols-2 w-full gap-2"
                                >
                                    <InputAutoComplete
                                        skip
                                        :error="v$.event_country.$error"
                                        label="Country"
                                        v-model="formData.event_country"
                                        :suggestion_data="
                                            countryList.countryList
                                        "
                                    />
                                    <InputAutoComplete
                                        :skip="!formData.is_migrant"
                                        :error="v$.event_province.$error"
                                        label="Province"
                                        @change="
                                    ...
                                    <InputAutoComplete
                                        :skip="!formData.is_migrant"
                                        :error="v$.event_municipality.$error"
                                        label="Municipality"
                                        v-model="formData.event_municipality"
                        ...
                        <div class="grow">
                            ...
                            >
                                <div
                                    class="grid grid-cols-1 w-full gap-2 sm:mt-5"
                                >
                                    <Input
                    ...
                    <div class="flex flex-row flex-wrap gap-2">
                        ...
                        <div
                            class="grow"
                            v-if="formData.petition_type === 'CCE'"
                        >
                            <Box
                            ...
                            >
                                <div
                                    class="flex flex-col gap-2 w-full font-bold relative"
                                >
                                    <div
                                ...
                                <div class="flex flex-col gap-2 w-full mt-5">
                                    <p class="text-xs text-gray-300">
                                        {{ formData.clerical_errors.length }}
                                    </p>

                                    <draggable
                                        v-model="formData.clerical_errors"
                                        item-key="error_num"
                                        handle=".drag-handle"
                                        animation="200"
                                    ...
                                    >
                                        <template #item="{ element, index }">
                                            ...
                                            >
                                                <!-- drag icon -->
                                                <div
                                                    class="cursor-move drag-handle text-gray-400 px-2"
                                                >
                                                    <font-awesome-icon
                            ...
                            nolabel /> -->
                                                ...
                                                <div class="grow">
                                                    <Input
                                                        ...
                                                        cap
                                                        v-model="
                                                            element.error_description_from
                                                        "
                                                    />
                                                </div>
                                    ...
                                    >
                                        <div class="w-auto -top-4 right-4">
                                            <p
                                                class="text-sm italic text-gray-400 font-normal"
                                            >
                                                <font-awesome-icon
                                                    icon="fa-solid fa-circle-info"
                                                    class="me-1 text-blue-600"
                                                />
                                                <kbd>Ctrl</kbd> +
                                                <kbd>Space</kbd> to add new
                                                <strong>row</strong>
                                            </p>
                                        </div>
                                        <button
                                            type="button"
                                            @click="add_clerical_error()"
                        ...
                        >
                            ...
                            >
                                <div class="grid grid-cols-2 gap-2 w-full">
                                    <Input
                                        :error="v$.first_name_from.$error"
                                        label="From"
                                        @change="generate_granted_text()"
                                        @input="
                                            formData.first_name_from =
                                                $event.target.value.toUpperCase()
                                        "
                                        v-model="formData.first_name_from"
                                    />
                                    <Input
                                        :error="v$.first_name_to.$error"
                                        @change="generate_granted_text()"
                                        @input="
                                            formData.first_name_to =
                                                $event.target.value.toUpperCase()
                                        "
                                        label="To"
                        ...
                        >
                            ...
                            >
                                <div
                                    class="flex flex-col gap-6 text-md font-base justify-center"
                                >
                                    <div
                                        class="flex flex-row gap-2 items-center"
                                    >
                                    ...
                                    >
                                        ...
                                        <div class="relative">
                                            I have/He/She has habitually and
                                            continuously used
                                            <div
                                                class="absolute -top-[1.2rem] left-[23rem]"
                                            >
                                                <Input
                                                    :error="
                                                        v$.ground_b_data.$error
                                                    "
                                                    cap
                                                    nolabel
                                                    :class="`flex text-center`"
                                                    v-model="
                                                        formData.ground_b_data
                                                    "
                                                    :readonly="
                                                        formData.ground_b
                                                            ? false
                                            ...
                                            &nbsp;and I/He/She is publicly known
                                            in the community with that first
                                            name;
                                        </div>
                                    </div>
                                    <div
                                    ...
                                    >
                                        ...
                                        <div for="one">
                                            ...
                                            >
                                                The first name is tainted with
                                                dishonor;</label
                                            >
                                        </div>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <div class="relative w-max">
                                            Others: (Specify)
                                            <div
                                                class="absolute -top-[1.2rem] left-[7.4rem] w-full"
                                            >
                                                <Input
                                                    :error="
                                                        v$.ground_f_data.$error
                                                    "
                                                    nolabel
                                                    v-model="
                                                        formData.ground_f_data
                                                    "
                                                    :class="`flex text-center`"
                                                    :readonly="
                                                        formData.ground_f
                        ...
                        <div
                            class="basis-[100%]"
                            v-if="formData.petition_type !== 'CFN'"
                        >
                            <Box
                            ...
                            >
                                ...
                                >
                                    <div
                                        ...
                                        v-for="(
                                            value, index
                                        ) in formData.clerical_errors"
                                        :key="index"
                                    >
                                        <div
                                            class="basis-[20%] px-8 flex items-start justify-start flex-col"
                                        >
                                            <label
                                                :for="index"
                                ...
                                >
                                    <TextArea
                                        :rows="4"
                                        label="Facts/Reasons"
                                        v-model="formData.reasons[0].reason"
                                    />
                                </div>
                            </Box>
                        </div>
                    ...
                    <div
                        class="flex sm:flex-col md:lg:flex-row flex-wrap gap-2"
                    >
                        <div class="grow">
                            <Box
                            ...
                            >
                                <div
                                    class="flex flex-col w-full gap-2 mt-5 relative"
                                >
                                    <div class="absolute w-auto -top-9 right-4">
                                        <p
                                            class="text-xs italic text-gray-400 font-normal"
                                        >
                                            <font-awesome-icon
                                                icon="fa-solid fa-circle-info"
                                                class="me-1 text-blue-600"
                                    ...
                                    >
                                        <p
                                            class="basis-[9%] text-sm text-center"
                                        >
                                            {{ indexToLetter(index) }} )
                                        </p>
                                        <div class="basis-[90%]">
                                    ...
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <button
                                            ...
                                            "
                                            v-if="supporting_items.length > 1"
                    ...
                    <!-- 9th  Page 2 Starts-->
                    <div
                        class="relative border-b-2 border-dashed border-yellow-400 flex items-center justify-center mt-10 mb-10"
                    >
                        <p
                            class="absolute bg-white px-2 font-semibold italic text-gray-800"
                        >
                            PAGE 2
                        </p>
                    </div>
                    ...
                    <div
                        class="flex flex-wrap sm:flex-col md:lg:flex-row gap-2"
                    >
                        <div class="basis-[50%]">
                            ...
                            <Box title="ADMINISTERING OFFICER" width="w-auto">
                                <div class="grid grid-cols-1 w-full gap-2">
                                    <Input
                                        ...
                                        "
                                        :error="
                                            v$.administering_officer_name.$error
                                        "
                                        cap
                                    />
                                    <Input
                        ...
                        <div class="grow">
                            <Box title="SUBSCRIBE AND SWORN" width="w-auto">
                                <div
                                    class="grid sm:grid-cold-1 md:lg:grid-cols-3 w-full gap-2"
                                >
                                    <Input
                                        label="Date Sworn"
                                        type="date"
                                        skip
                                        v-model="formData.subscribe_sworn_date"
                                    ...
                                    <Input
                                        ...
                                        skip
                                        v-model="
                                            formData.subscribe_sworn_city_municipality
                                        "
                                        :error="
                                            v$.subscribe_sworn_city_municipality
                                                .$error
                                        "
                                    />
                                    <Input
                                        label="exhibiting his/her"
                                        v-model="formData.exhibiting_his_her"
                                        :error="v$.exhibiting_his_her.$error"
                                    />
                                    <Input
                                        :label="
                                            formData.exhibiting_his_her || `No.`
                                        "
                                        v-model="formData.exhibiting_number"
                    ...
                    <div class="flex flex-row gap-2">
                        ...
                        v-model="formData.petition_actions[0].action_decision" /> -->
                                    ...
                                    <div
                                        v-if="
                                            formData.republic_act_number ===
                                                '10172' && !formData.is_migrant
                                        "
                                        class="flex flex-col"
                                        v-for="(
                                            value, index
                                        ) in clerical_errors_items"
                                        :key="index"
                                    >
                                        <div
                        ...
                        v-model="formData.petition_actions[index].action_decision" /> -->
                                    ...
                                    >
                                        <Input
                                            ...
                                            label="Date"
                                            skip
                                            v-model="formData.action_taken_date"
                    ...
                    <div class="flex sm:flex-col md:lg:flex-row gap-2">
                        <div class="flex flex-col gap-2 basis-[35%]">
                            <Box title="Payment of filing fee" width="w-auto  ">
                                ...
                                >
                                    <div class="grid grid-cols-1 w-full gap-2">
                                        ...
                                        <Input
                                            :readonly="formData.is_indigent"
                                            :skip="formData.is_indigent"
                                            label="O.R. No."
                                            type="text"
                            ...
                            >
                                <div
                                    class="grid grid-cols-1 mt-4 w-full h-full gap-2"
                                >
                                    <!-- <p class="text-xs italic">Optional, notice for publication</p> -->
                                    <Input
                                        label="Father Name"
                                        v-model="formData.name_father"
                                    />
                                    <Input
                                        label="Mother Name"
                                        v-model="formData.name_mother"
                                    />

                                    <Input
                                        label="Name of Newspaper"
                                        skip
                                        v-model="formData.publication_newspaper"
                                    />
                                    <Input
                                        label="Place of Publication"
                                        skip
                                        v-model="formData.publication_place"
                                    />
                                </div>
                            </Box>
                        </div>
                            ...
                            <Box title="DATE Configuration" width="w-auto">
                                ...
                                >
                                        ...
                                        <div class="flex items-center">
                                            <div class="w-auto">
                                                <Input
                                                    ...
                                                    type="date"
                                                    v-model="
                                                        formData.petition_date_issued
                                                    "
                                                    :error="
                                                        v$.petition_date_issued
                                                            .$error
                                                    "
                                                />
                                            </div>
                                    ...
                                    >
                                        <Input
                                            skip
                                            :error="v$.publication_start.$error"
                                        ...
                                        <Input
                                            ...
                                            label="And"
                                            type="date"
                                            v-model="formData.publication_and"
                                        />
                                        <Input
                                            skip
                                            :error="v$.publication_end.$error"
            ...
            <template v-slot:footer>
                <div
                    class="w-full flex items-center justify-between px-4 py-2 bg-white border-t"
                >
                    <!-- Left: Validation toggle -->
                    <div class="flex items-center gap-2">
                        <CheckBox
                            skip
                            v-model="formData.is_to_validate"
                            tabindex="-1"
                        />
...
import {
    ...
    complete_municipality_with_province_with_words,
    complete_province
...
const municipality_of = computed(() => {
    return complete_municipality_with_province_with_words(
...
const open_modal = async () => {
    ...
    if (cce) {
        ...
        is_default_petitioner_number.value = (parseInt(latest[1], 10) + 1)
            .toString()
            .padStart(4, '0')
    } else {
...
function PublicationSetter() {
    const isCFNorRA10172 =
        formData.petition_type === 'CFN' ||
        formData.republic_act_number === '10172'

    const publicationDefaults = {
        publication_start: isCFNorRA10172
            ? add_publication_start().toString()
            : '',
        publication_and: isCFNorRA10172 ? add_publication_and().toString() : '',
        publication_end: isCFNorRA10172 ? add_publication_end().toString() : '',
        petition_date_granted: isCFNorRA10172
            ? add_date_granted_with_publication().toString()
            : add_date_granted().toString(),
...
const petitioner_number = computed(() => {
    const is_r_a_10172 =
        formData.republic_act_number === '10172' ? 'R.A. 10172' : ``
    const petition_check = formData.is_migrant
        ? 'MP' + formData.petition_type
        : formData.petition_type
    let combined =
        petition_check +
        '-' +
        is_default_petitioner_number.value +
        '-' +
        is_default_petitioner_year.value +
        ' ' +
        is_r_a_10172
    return combined
})
...
const document_type_label = computed(() => {
    return formData.event_type === 'Birth'
        ? 'Live Birth'
        : formData.event_type === 'Death'
          ? 'Death'
          : formData.event_type === 'Marriage'
...
const next_input = (event, to_add) => {
    ...
    const inputs = Array.from(
        document.querySelectorAll('input, button, [tabindex]')
...
function change_event_selected_error_in() {
    ...
    formData.event_type === 'Birth'
        ? ((formData.petitioner_error_in = 'my'),
          (formData.document_owner = 'N/A'),
          (formData.relation_owner = 'N/A'))
        : formData.event_type === 'Death'
          ? ((formData.petitioner_error_in = 'the'),
            (formData.document_owner = ''),
...
function change_migrant() {
    if (formData.is_migrant) {
        ...
        formData.action_taken_date = add_date_granted().toString()
        formData.header_province =
            system_setting.defaults.province.toUpperCase() || ''
        formData.header_municipality =
            'MUNICIPALITY OF ' +
                system_setting.defaults.municipality.toUpperCase() || ''
    }
}
...
const initialForm = {
    ...
    supporting_documents: [{ document_name: '' }],
...
const rules = computed(() => {
    return {
        ...
        administering_officer_name: {
            requiredIf: requiredIf(() => (!formData.is_migrant ? true : false))
        },
        ...
        ground_b_data: {
            requiredIf: requiredIf(() =>
                formData.ground_b && formData.petition_type === 'CFN'
                    ? true
                    : false
            )
        },
        ground_f_data: {
            requiredIf: requiredIf(() =>
                formData.ground_f && formData.petition_type === 'CFN'
                    ? true
                    : false
            )
        },
...
const create_validated_document = async () => {
    ...
    const is_busy = await window.ClericalApi.IsFileBusy(
        last_saved_filepath.value + 'petition.docx'
    )

    if (is_busy) {
        alertmodal.value = true
        alertmodal_body.value =
            'The file is currently open. Please save and close the file before proceeding.'
    ...
    const submit_ =
        is_document_edit_mode.value || is_document_regenerating.value
...
const mapDataToForm = (data) => {
    ...
    formData.administering_officer_position =
        data.administering_officer_position
    ...
    formData.subscribe_sworn_city_municipality =
        data.subscribe_sworn_city_municipality
...
const cancel_regen_stage = async () => {
    ...
    if (is_busy) {
        alertmodal.value = true
        alertmodal_body.value =
            'The file is currently open. Please close the file before proceeding.'
        alertmodal_title.value = 'File Busy'
