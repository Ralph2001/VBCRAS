<template>
    <div class="w-full h-screen flex items-center justify-center">
        <div
            class="fixed top-0 left-0 right-0 justify-center items-center flex border w-full h-12 bg-white z-50"
        >
            <p class="text-lg font-bold text-gray-700 font-mono uppercase f">
                {{ auth.user_role === 1 ? 'SYSTEM SETUP' : ':(' }}
            </p>
        </div>
        <div
            v-if="auth.user_role === 1"
        ...
        >
            <div class="mt-10 flex flex-col lg:px-20 h-full">
                <div class="flex flex-col">
                    <p
                        class="font-bold text-xl text-gray-700 uppercase font-mono"
                    >
                        Default Saving Path for all files
                    </p>
                    <p class="text-sm text-gray-700 mt-2">
                        This will be the default saving path for all generated
                        documents. Please ensure that the location is a shared
                        drive if you plan to share your data with others so they
                        can access and open the generated documents as well.
                        Thank you!
                    </p>

                    <div class="w-full flex flex-col md:lg:grid-cols-2 mt-2">
                        <div
                            class="flex flex-row gap-2 w-full items-center justify-center mt-4"
                        >
                            <input
                                type="text"
                                ...
                                readonly
                                :error="v$.file_path.$error"
                                v-model="formData.file_path"
                            />

                            <button
                                class="border rounded p-2 mt-auto font-medium bg-blue-500 text-white px-4"
                            ...
                            <button
                                class="border rounded p-2 mt-auto font-medium bg-red-400 text-white px-4"
                                v-if="formData.file_path"
                    ...
                    <div
                        class="flex flex-col mt-6 gap-2"
                        v-if="formData.file_path"
                    >
                        <p
                            class="font-mono font-semibold text-gray-600 text-sm"
                        >
                            Path Preview:
                        </p>

                        <div
                            class="w-full py-1.5 px-2 rounded-sm bg-gray-50 flex items-center overflow-hidden"
                        >
                            <p class="text-gray-700 font-mono text-sm truncate">
                                <span
                                    class="hover:text-blue-500"
                                    :title="formData.file_path"
                                    >...</span
                                >
                                >
                                <span class="text-blue-500" title="System Name"
                                    >VBCRAS</span
                                >
                                > Corrections > Year > Month > Document Name
                            </p>
                        </div>
                        <div
                            class="w-full py-1.5 px-2 rounded-sm bg-gray-50 flex items-center overflow-hidden"
                        >
                            <p class="text-gray-700 font-mono text-sm truncate">
                                ...
                                <span class="text-blue-500" title="System Name"
                                    >VBCRAS</span
                                >
                                > Forms > Form Type > Year > Month > Document
                                Name
                            </p>
                        </div>
                        <div
                        ...
                        >
                            <p class="text-gray-700 font-mono text-sm truncate">
                                ...
                                <span class="text-blue-500" title="System Name"
                                    >VBCRAS</span
                                >
                                > Application for Marriage License > Year >
                                Month > Document Name
                            </p>
                        </div>
                ...
                <div class="w-full flex flex-col">
                    <p
                        class="font-bold text-xl text-gray-700 uppercase font-mono mt-10 mb-4"
                    >
                        Default Value
                    </p>
        ...
        <div
            v-if="auth.user_role === 2"
            class="flex flex-col items-center justify-center h-full w-full p-8 text-center bg-white"
        >
            <div class="mb-8">
            ...
            <p class="text-lg text-gray-600 max-w-lg">
                The system needs to be configured by an administrator before you
                can proceed.
            </p>
            ...
            <div class="mt-12">
                <button
                    @click="auth.logout"
                    class="px-8 py-3 bg-gray-200 text-gray-800 rounded-lg hover:bg-red-500 hover:text-white transition duration-300 ease-in-out shadow-md"
                >
                    Sign Out
                </button>
            </div>
