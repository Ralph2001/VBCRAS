<template>
  <div class="flex flex-col relative justify-center w-full p-10 CCEMAIN ">
    ...
    <Modal large label="Create a new Document" footerBG="bg-slate-300" v-if="document"
      ...
      <div class="flex flex-col sm:px-4 md:lg:px-[20rem] h-max w-full gap-4 relative bg-slate-400" :style="bgTexture">
                    ...
                    <div class="flex flex-col w-full gap-6">
                      ...
                      <div class="grid grid-cols-1 w-full gap-2">
                        <textarea :id="`action_taken${index}`" v-model="formData.action_taken.decision[index]" :class="{
                          ...
                          class="block p-2.5 text-justify font-semibold px-2.5 tracking-wider w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
                      </div>
                    </div>
                  </div>
...
const submitForm = async () => {
  ...
  // const check = await window.ClericalApi.createPetitionDocument(data);
