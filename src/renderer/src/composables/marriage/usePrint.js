...
export function usePrint(active_pdf_link, active_pdf, count) {
    ...
    const printMethods = [
        { id: 'method1', name: 'Most Accurate Printing', description: 'Time consuming, ideal for precise layouts.', icon: 'fa-solid fa-print' },
        { id: 'method2', name: 'Accurate', description: 'Sometimes multi-page documents are not selectable.', icon: 'fa-solid fa-print' },
        { id: 'method3', name: 'Easy to Use', description: 'Some printers have inaccurate page sizes.', icon: 'fa-solid fa-print' },
    ];
    ...
    const printPDF = async () => {
        ...
        if (selectedPrintMethod.value === 'method1') {
            ...
            try {
                await window.LocalCivilApi.printPDF(printBase64.value, '', '', selectedPrintMethod.value);
            } catch (error) {
